    ...
    try:
        ...
        for ctrls in controllers:
            if str(ctrls['callsign']).startswith(icao):
                ctrlsdatacount += 1
                ctrlsdata.insert(ctrlsdatacount, f"{ctrls['callsign']}:{ctrls['frequency']}")
...
async def sendvatsimembed(ctrlsdata: list, icao: str):
    ...
    # Loop thru all positions online and split observers/supervisors online out of picture
    for x in ctrlsdata:
        splitx = str(x).split(':')
        if splitx[1] != '199.998':
            embed.add_field(name=splitx[0], value=splitx[1])
    return embed
